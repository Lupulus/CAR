...
public class DirRessource extends AbstractRessource {
	...
	/**
	 * Liste le contenu du répertoire path
	 * @param path Chemin du répertoire
	 * @param auth (Authentification nécessaire)
	 * @param transfertMode Mode de transfert (active/passive)
	 * @return Retourne la réponse au format JSON contenant la liste des fichiers avec 
	...
	public Response getDir(@PathParam("path") String path, @HeaderParam("Authorization") String auth,
		...
		return new FtpExec()
				...
				.addAction((exec) -> {
					...
					FTPFile[] files = cl.listFiles(absPath); // récupération du contenu du dossier depuis le serveur
	 ...
	 * @param auth (Authentification nécessaire)
	 * @param transfertMode mode de transfert (active/passive)
	 * @return Retourne une reponse dans un objet JSON : Si l'action c'est mal passée, un code d'erreur est renvoyé. 
	 */
	@DELETE
	@Path("{path:.+}")
	 ...
	 * @return Retourne une reponse dans un Objet JSON. Si l'action c'est mal passée, un code d'erreur HTTP 
	 * ainsi qu'un code d'erreur FTP est renvoyé. 
	 */
	@POST
	@Path("{path:.+}")
